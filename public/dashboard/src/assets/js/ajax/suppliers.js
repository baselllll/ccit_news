$(document).ready(function($){
    ...
    $(document).on('click', '.changeStatus', function (event) {
        ...
        if (active)
        {
            changeStatus(uuid, 0, 'تعطيل المورد');
        }else {
            changeStatus(uuid, 1, 'تفعيل المورد');
        }
    });
    ...
    function getOne(showUrl)
    {
        $.ajax({
            ...
            .done(function (data) {
                $('#name_edit').val(data.supplier.name);
                $('#taxNumber_edit').val(data.supplier.taxNumber);
                $('#phone_edit').val(data.supplier.phone);
                $('#additional_phone_edit').val(data.supplier.additional_phone);
                $('#address_edit').val(data.supplier.address);
                if (data.supplier.active === 1)
                {

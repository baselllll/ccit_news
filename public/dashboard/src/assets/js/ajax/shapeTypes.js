$(document).ready(function($){
    ...
    var table = $('#shape-types-table').DataTable( {
    ...
    function getOne(showUrl)
    {
        $.ajax({
            ...
            .done(function (data) {
                $('#name_edit').val(data.shapType.name);
                $('#priority_edit').val(data.shapType.priority);
                if (data.shapType.active === 1)
                {
                    $('#active_edit').attr('checked',true);
                }else {
                    $('#active_edit').attr('checked',false);
                }
                $('#id').val(data.shapType.id);
                $('#uuid').val(data.shapType.uuid);
            })
            .fail(function (res) {
                Snackbar.show({
                    text: res.responseJSON.message,
    ...
    $(document).on('click', '.changeStatus', function (event) {
        ...
        if (active)
        {
            changeStatus(uuid, 0, 'تعطيل نوع الشكل');
        }else {
            changeStatus(uuid, 1, 'تفعيل نوع الشكل');
        }
    });

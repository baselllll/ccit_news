$(document).ready(function($){
    ...
    var table = $('#voucher-receipts-table').DataTable( {
    ...
    function getOne(showUrl)
    {
        $.ajax({
            ...
            .done(function (data) {
                $('#customer_name_edit').val(data.receipt.customer_name);
                $('#price_edit').val(data.receipt.price);
                $('#price_text_edit').val(data.receipt.price_text);
                $('#for_edit').val(data.receipt.for);
                $('#id').val(data.receipt.id);
                $('#uuid').val(data.receipt.uuid);
                if (data.receipt.payment_type === 'cash')
                {
                    $('#cash_edit').attr('checked',true);
                }else if(data.receipt.payment_type === 'check'){
    ...
    function preparePrint(showUrl)
    {
        $.ajax({
            ...
            .done(function (data) {
                $('#paymentID').text(data.receipt.id);
                $('#paymentDate').text(data.payment_date);
                $('#customerName').text(data.receipt.customer_name);
                $('#priceText').text(data.receipt.price_text);
                $('#paymentType').text(data.payment_type);
                $('#andThatForPrint').text(data.receipt.for);
            })
            .fail(function (res) {
                Snackbar.show({
                    text: res.responseJSON.message,

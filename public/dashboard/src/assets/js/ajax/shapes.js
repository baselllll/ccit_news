$(document).ready(function($){
    ...
    var table = $('#shapes-table').DataTable( {
        ...
        "columns": [
            {data:'name', name:'name'},
            {data:'shapeImage', name:'shapeImage', orderable:false, searchable:false},
            {data:'code', name:'code'},
            {data:'shapeTypeName', name:'shapeTypeName', orderable:false, searchable:false},
    ...
    function getOne(showUrl)
    {
        $.ajax({
            ...
            .done(function (data) {
                $('#name_edit').val(data.shape.name);
                $('#code_edit').val(data.shape.priority);
                $('#id').val(data.shape.id);
                $('#uuid').val(data.shape.uuid);
                $('#imageShow').attr('src', data.shape.image).show();
                $("#selectShapeType_edit option").each(function(k, val)
                {
                    if ($(this).val() === data.shape.shape_type_id)
                    {
                        let select = document.getElementById('selectShapeType_edit');
                        let instance = select.tomselect;
                        instance.addItem($(this).val())
